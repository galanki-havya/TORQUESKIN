...
const Home = () => {
  ...
  return (
    ...
    >
      ...
      <section className="relative w-full h-[90vh] md:h-screen flex items-center pt-20 md:pt-32 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src={heroImage} className="w-full h-full object-cover object-[70%_center] md:object-right opacity-90" alt="Torqueskin" />
          <div className="absolute inset-0 bg-gradient-to-r from-[#0b0b14] via-[#0b0b14]/60 md:via-[#0b0b14]/40 to-transparent z-10" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0b0b14] via-transparent to-transparent z-10" />
        </div>
        
        <div className="relative z-20 w-full px-6 md:pl-8 lg:pl-12">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9 }} className="max-w-xl text-left">
            ...
            <h2 className="text-base md:text-xl font-bold text-gray-200 mt-3 italic uppercase tracking-wide">Ultimate Vehicle Protection</h2>
            <p className="mt-5 text-[11px] md:text-sm text-white font-bold tracking-wide max-w-xs md:max-w-sm border-l-4 border-[#d4af37] pl-4">
              Advanced TPU Paint Protection Films and Nano-Ceramic Window Tints. Engineered for Performance.
            </p>
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              <button 
                onClick={() => navigate('/products')}
                className="bg-[#d4af37] text-black px-8 py-4 rounded-sm font-black uppercase tracking-widest text-[10px] hover:bg-white transition-all flex items-center justify-center gap-2"
              >
              ...
              <button 
                ...
                className="border-2 border-white/50 px-8 py-4 rounded-sm font-black uppercase tracking-widest text-[10px] text-white hover:bg-white/10 transition-all text-center"
              >
                Book Installation
              </button>
            </div>
      ...
      <section className="py-10 md:py-12 bg-white/5 backdrop-blur-xl border-y border-white/10 relative z-30">
      ...
      <section className="py-20 md:py-24 relative bg-white/[0.03] border-y border-white/10 overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 select-none hidden md:flex">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center justify-center gap-10 md:gap-16">
            ...
            <div className="bg-black/40 border border-white/20 p-6 md:p-10 rounded-2xl w-full lg:max-w-lg backdrop-blur-xl shadow-2xl">
              <h2 className="text-2xl md:text-3xl font-black uppercase italic mb-6 text-white">The <span className="text-[#d4af37]">Science</span> of Safety</h2>
              <p className="text-gray-200 text-xs md:text-sm leading-relaxed mb-8 font-semibold italic">Torqueskin PPF is a multi-layer molecular shield engineered for extreme road conditions.</p>
      ...
      <section className="py-16 md:py-20 relative">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-10 md:mb-12">
            ...
            <p className="text-gray-400 text-[9px] md:text-xs uppercase tracking-[0.3em] md:tracking-[0.4em] mt-3 font-black">Superior Engineering vs Industry Standards</p>
          </div>
          ...
          <div className="max-w-6xl mx-auto overflow-hidden rounded-2xl border border-white/10 bg-black/60 backdrop-blur-md shadow-2xl">
            ...
            <div className="md:hidden divide-y divide-white/10">
              <MobileComparisonRow label="Healing Tech" standard="Heat Required" torque="Instant Ambient Healing" />
              <MobileComparisonRow label="Thickness" standard="150 Microns" torque="195+ Microns (HD)" />
              <MobileComparisonRow label="Surface Energy" standard="Low (Holds Dirt)" torque="High Hydrophobic" />
              <MobileComparisonRow label="Warranty" standard="3-5 Years" torque="Up to 10 Years" />
            </div>
          </div>
        </div>
      </section>

      <section className="py-16 md:py-20 max-w-7xl mx-auto px-6 border-t border-white/10">
        <h2 className="text-2xl md:text-4xl font-black uppercase italic tracking-tighter mb-10 md:mb-12 text-white border-b-4 border-[#d4af37] pb-2 inline-block">
...
const MobileComparisonRow = ({ label, standard, torque }) => (
  <div className="p-6 space-y-4">
    <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[#d4af37]">{label}</div>
    <div className="grid grid-cols-2 gap-4">
      <div className="flex flex-col gap-1">
        <span className="text-[8px] text-gray-500 uppercase font-bold">Standard</span>
        <div className="flex items-center gap-2 text-xs text-gray-400">
          <XCircle size={14} className="text-red-500/40" /> {standard}
        </div>
      </div>
      <div className="flex flex-col gap-1">
        <span className="text-[8px] text-[#d4af37] uppercase font-bold">TorqueSkin</span>
        <div className="flex items-center gap-2 text-xs text-white italic font-bold">
          <CheckCircle2 size={14} className="text-[#d4af37]" /> {torque}
        </div>
      </div>
    </div>
  </div>
...
const StepCard = ({ number, icon, title, desc }) => (
  <div className="relative p-6 md:p-8 border border-white/20 bg-white/5 rounded-xl group hover:border-[#d4af37] transition-all duration-300">
    <div className="text-[#d4af37] mb-4 group-hover:scale-110 transition-transform">{icon}</div>
    <span className="absolute top-4 right-6 text-4xl md:text-5xl font-black text-white/5 italic select-none">{number}</span>
    <h4 className="text-xs md:text-sm font-black uppercase italic text-white mb-2 tracking-widest">{title}</h4>
    <p className="text-gray-300 text-[9px] md:text-[10px] leading-relaxed font-bold uppercase tracking-tight">{desc}</p>
  </div>
);
...
const StatItem = ({ icon, label, value }) => (
  <div className="flex flex-col items-center text-center space-y-2 group">
    ...
    <div className="text-xl md:text-2xl font-black italic text-white tracking-tighter">{value}</div>
    <div className="text-[9px] md:text-[10px] uppercase tracking-widest text-gray-300 font-black">{label}</div>
  </div>
);
...
const LayerInfo = ({ step, title, detail }) => (
  <div className="flex gap-4 items-start border-b border-white/10 pb-4 group last:border-0">
    ...
    <div>
      <h4 className="font-black uppercase text-[9px] md:text-[10px] tracking-[0.15em] text-white leading-none">{title}</h4>
      <p className="text-gray-200 text-[11px] md:text-xs mt-2 font-semibold leading-relaxed">{detail}</p>
    </div>
  </div>
);

const ProductCard = ({ title, icon, desc, features }) => (
  <div className="bg-black/40 border border-white/20 p-8 md:p-10 hover:border-[#d4af37] transition-all shadow-2xl backdrop-blur-md group relative h-full">
    <div className="mb-6 group-hover:scale-110 transition-transform duration-300">{icon}</div>
    <h3 className="text-xl md:text-2xl font-black uppercase italic mb-3 text-white">{title}</h3>
    <p className="text-gray-200 text-[11px] md:text-xs mb-8 font-semibold leading-relaxed italic">"{desc}"</p>
    <div className="space-y-3 border-t border-white/10 pt-6">
      {features.map((f, i) => (
        <div key={i} className="flex items-center gap-3 text-[9px] md:text-[10px] font-black uppercase text-white tracking-widest">
