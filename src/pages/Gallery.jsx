...
const Gallery = () => {
  ...
  const products = [
    { 
      ...
      span: "lg:col-span-8 h-[350px] md:h-[500px]",
    ...
    { 
      ...
      span: "lg:col-span-4 h-[350px] md:h-[500px]",
    ...
    { 
      ...
      span: "lg:col-span-4 h-[350px] md:h-[400px]",
    ...
    <div className="min-h-screen bg-[#0b0b14] text-white font-sans selection:bg-[#d4af37]" 
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            <button className="absolute top-6 right-6 md:top-10 md:right-10 text-[#d4af37] hover:scale-110 transition-transform">
              <X size={32} md:size={40} strokeWidth={1} />
            </button>
            <motion.img 
              initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              src={selectedImage} 
              className="max-w-full max-h-[80vh] md:max-h-[85vh] object-contain border border-white/10 shadow-2xl"
      ...
      <div className={`min-h-screen w-full bg-black/50 pt-24 md:pt-32 pb-10 md:pb-20 px-4 md:px-6 transition-all duration-700 ${selectedImage ? 'blur-md' : ''}`}>
          ...
          <AnimatePresence mode="wait">
            {!activeAlbum ? (
              <motion.div key="grid" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 md:gap-10 mb-12 md:mb-20">
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 text-[#d4af37]">
                        ...
                        <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] md:tracking-[0.8em]">TorqueSkin Professional Logs</span>
                    </div>
                    <h1 className="text-5xl md:text-8xl font-black italic uppercase tracking-tighter leading-none">
                      THE<span className="text-[#d4af37] text-outline ml-2 md:ml-4">GALLERY.</span>
                    </h1>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    {categories.map((cat) => (
                      <button 
                        ...
                        className={`flex-1 md:flex-none px-4 md:px-8 py-2 md:py-3 text-[9px] md:text-[10px] font-bold uppercase tracking-widest border transition-all duration-500 ${filter === cat ? 'bg-[#d4af37] border-[#d4af37] text-black shadow-lg shadow-[#d4af37]/20' : 'border-white/10 text-gray-500 hover:text-white bg-black/20'}`}
                      >
                        {cat}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
                  {filteredProducts.map((product) => (
                    ...
                    >
                      <img src={product.src} alt={product.title} className="w-full h-full object-cover opacity-70 group-hover:opacity-20 group-hover:scale-110 transition-all duration-[1.5s]" />
                      <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-between z-20">
                        ...
                        <div className="space-y-1 md:space-y-2">
                          <span className="text-[#d4af37] text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em]">{product.warranty}</span>
                          <h3 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter leading-none">{product.title}</h3>
                        </div>
                      </div>
                    </motion.div>
                  ))}
              ...
              <motion.div key="album" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                <button onClick={() => setActiveAlbum(null)} className="group flex items-center gap-3 text-[#d4af37] mb-8 md:mb-12 font-bold uppercase tracking-[0.3em] md:tracking-[0.4em] text-[9px] md:text-[10px] hover:text-white transition-all">
                  <ChevronLeft size={18} className="group-hover:-translate-x-2 transition-transform" /> Back to Gallery
                </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16">
                  ...
                  <div className="lg:col-span-4 space-y-8 md:space-y-10 order-2 lg:order-1">
                    <div className="border-l-2 border-[#d4af37] pl-4 md:pl-6">
                      <span className="text-[#d4af37] text-[9px] md:text-[10px] font-black uppercase tracking-[0.5em]">{activeAlbum.category}</span>
                      <h2 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter mt-2 md:mt-4 leading-none">{activeAlbum.title}</h2>
                    </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 pt-2">
                        {activeAlbum.features.map((feature, i) => (
                          <div key={i} className="flex items-center gap-3 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-white/80">
                    ...
                    <div className="p-5 md:p-6 bg-white/5 border border-white/10 rounded-sm">
                        <div className="flex items-center gap-4">
                            <Clock className="text-[#d4af37]" size={20}/>
                            <div>
                                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white/40">Duration</p>
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-transparent border border-[#d4af37]/30 hover:bg-[#d4af37] hover:text-black transition-all duration-500 text-[10px] font-black uppercase tracking-[0.3em] active:scale-95"
                    >
                      Book This Treatment
                    </button>
                  </div>

                  <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 h-fit order-1 lg:order-2">
                    {activeAlbum.album.map((img, index) => (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 md:group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <Maximize2 size={24} className="text-[#d4af37] md:scale-50 md:group-hover:scale-100 transition-transform" />
                        </div>
                      </motion.div>
                    ))}
                  </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media (min-width: 768px) {
          .text-outline { -webkit-text-stroke: 1.5px rgba(212, 175, 55, 0.4); }
        }
      `}} />
    </div>
  );
